...
job "example" {
  ...
  #
  1. 既然要更新, 那么就决定一次更新需要更新多少个, 逐步更新
  2. 更新后是否成功怎么判断, 达到healthy状态最少min_healthy_time后则表示该job更新成功, 如果成功则可以继续更新
  3. 如果不成功则等待healthy_deadline, 进行下一步操作, 例如回滚
  4. progress_deadline表示从整个过程开始, 如果一个task更新成功则刷新, 否则到时间后会表示这次update失败
  update {
  ...
  group "cache" {
    ...
    #
    在interval时间内如果失败次数超过attempts次, 则会由mode决定, fail会直接将改节点标记为失败, delay则将在下次interval时候再次尝试restart
    restart {
